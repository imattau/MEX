...
async fn match_log_root(client: &reqwest::Client, http_addr: std::net::SocketAddr) -> LogRootResponse {
    client.get(format!("http://{http_addr}/api/v1/match_log/root"))
        .header("X-API-Key", "dev-default-key")
        .send().await.unwrap()
        .json().await.unwrap()
}

async fn match_log_entries(client: &reqwest::Client, http_addr: std::net::SocketAddr) -> Vec<orderlog::LogEntry<engine::Match>> {
    client.get(format!("http://{http_addr}/api/v1/match_log/entries"))
        .header("X-API-Key", "dev-default-key")
        .send().await.unwrap()
        .json().await.unwrap()
}
...
async fn test_two_replicas_with_mixed_entry_points_converge_to_identical_match_history() {
    ...
    async fn submit(client: &reqwest::Client, http_addr: std::net::SocketAddr, req: serde_json::Value) -> SubmitOrderResponse {
        client.post(format!("http://{http_addr}/api/v1/order"))
            .header("X-API-Key", "dev-default-key")
            .json(&req)
            .send().await.unwrap()
            .json().await.unwrap()
    ...
    assert!(r1.success && r1.pending, "sell1 rejected by A: {:?}", r1.error);
    let r2 = submit(&client, b_http, sign_and_jsonify(&sk_buyer1, &buy1)).await;
    assert!(r2.success && r2.pending, "buy1 rejected by B: {:?}", r2.error);
    ...
    // claim to make here.
    wait_for_match_count(&client, a_http, b_http, 1, tokio::time::Instant::now() + per_pair_budget).await;
    ...
    assert!(r3.success && r3.pending, "sell2 rejected by B: {:?}", r3.error);
    let r4 = submit(&client, a_http, sign_and_jsonify(&sk_buyer2, &buy2)).await;
    assert!(r4.success && r4.pending, "buy2 rejected by A: {:?}", r4.error);
    let (a_match_root, b_match_root) = wait_for_match_count(&client, a_http, b_http, 2, tokio::time::Instant::now() + per_pair_budget).await;

    assert_eq!(a_match_root.len, 2, "replica A should have exactly 2 matches, not more (idempotency)");
    assert_eq!(b_match_root.len, 2, "replica B should have exactly 2 matches, not more (idempotency)");
    println!("Replica A match_log root: {} (len={})", hex::encode(a_match_root.root), a_match_root.len);
    println!("Replica B match_log root: {} (len={})", hex::encode(b_match_root.root), b_match_root.len);
    ...
    let a_matches = sorted_by_trade_ids(match_log_entries(&client, a_http).await.into_iter().map(|e| e.payload).collect());
    let b_matches = sorted_by_trade_ids(match_log_entries(&client, b_http).await.into_iter().map(|e| e.payload).collect());
    ...
    for (a_m, b_m) in a_matches.iter().zip(b_matches.iter()) {
        ...
        let a_traders = { let mut t = [a_m.maker_trader, a_m.taker_trader]; t.sort(); t };
        let b_traders = { let mut t = [b_m.maker_trader, b_m.taker_trader]; t.sort(); t };
        assert_eq!(a_traders, b_traders, "the same two traders must have been on both sides of the trade");
        ...
        println!("trade {:?}: timestamp_us delta = {ts_delta_us}us", canonical_order_pair(a_m));
        assert!(
