...
async fn main() {
    ...
    let target_addr: std::net::SocketAddr = args
        .get(1)
        .expect("usage: inject_bad_proof <target_addr> <target_node_id>")
        .parse()
        .unwrap();
    let target_id: u32 = args
        .get(2)
        .expect("missing target_node_id")
        .parse()
        .unwrap();
    ...
    let proof = BACKEND
        .prove_batch(&batch)
        .expect("proving a valid batch must succeed");
    ...
    assert!(
        !BACKEND.verify_proof(&proof, &tampered),
        "sanity check: tampering must actually break verification"
    );

    let injector = UdpTransport::bind("127.0.0.1:0".parse().unwrap(), None)
        .await
        .unwrap();
