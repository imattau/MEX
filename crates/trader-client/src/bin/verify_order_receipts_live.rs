...
async fn main() {
    ...
    let api_base = args
        .get(1)
        .expect("usage: verify_order_receipts_live <api_base_url>")
        .trim_end_matches('/')
        .to_string();
    ...
    assert!(
        !sell_receipt.is_null(),
        "resting order must still get a receipt"
    );
    ...
    assert!(
        !buy_receipt.is_null(),
        "matched order must also get its own receipt"
    );
    ...
    let bad_resp: serde_json::Value = http
        .post(format!("{api_base}/api/v1/order"))
        .header("X-API-Key", &api_key)
        .json(&bad_req)
        .send()
    ...
    assert_eq!(
        ...
        "corrupted-signature order should be rejected"
    );
    assert!(
        bad_resp["receipt"].is_null(),
        "rejected order must not get a receipt"
    );
    ...
    for (label, receipt) in [("sell", &sell_receipt), ("buy", &buy_receipt)] {
        ...
        assert!(
            orderlog::verify_receipt(&parsed),
            "{label} receipt failed independent signature verification"
        );
        println!(
            "{label} receipt independently verified against node_pubkey {}: OK",
            hex::encode(parsed.node_pubkey)
        );
    }
