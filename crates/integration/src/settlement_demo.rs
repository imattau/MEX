...
fn main() {
    ...
    for i in 0..3 {
        ...
        println!(
            "  Trader {} registered: PK {:02x}...{:02x} | MR rkey=0x{:04x}",
    ...
    println!(
        "  {:>6} {:>4} {:>8} {:>8}  {:>10}  {}",
        "Trader", "Side", "Price", "Amount", "Settle", "Requester"
    );
    println!("  {}", "─".repeat(52));
    ...
    println!(
        "\n  ✓ {} orders written to RDMA memory regions",
        all_signed_orders.len()
    );
    ...
    println!(
        "  Standard queue processed: {} batches",
        batches
            .iter()
            .filter(|b| {
                b.trades.first().map_or(false, |t| {
                    t.settlement_tier == SettlementPreference::Standard
                })
            })
            .count()
    );
    println!(
        "  Express queue processed: {} batches",
        batches
            .iter()
            .filter(|b| {
                b.trades.first().map_or(false, |t| {
                    t.settlement_tier == SettlementPreference::Express
                })
            })
            .count()
    );
    println!(
        "  Instant queue processed: {} batches",
        batches
            .iter()
            .filter(|b| {
                b.trades.first().map_or(false, |t| {
                    t.settlement_tier == SettlementPreference::Instant
                })
            })
            .count()
    );
    ...
    if !batches.is_empty() {
        ...
        println!(
            "  Sample batch: {} trades, value: {} USD, {} proofs ({} bytes total)",
            first.trades.len(),
            first.total_value,
            first.proofs.len(),
            proof_bytes
    ...
    let encrypted = encrypt_packet(&mesh_key, &batch_payload).expect("Encrypt failed");
    println!("  Cipher:    ChaCha20-Poly1305 (AEAD)");
    println!(
        "  Plaintext: {} bytes → Ciphertext: {} bytes (+12 nonce + 16 tag)",
        batch_payload.len(),
        encrypted.len()
    );
    let decrypted = decrypt_packet(&mesh_key, &encrypted).expect("Decrypt failed");
    ...
    for (_tier, count) in all_matches
        .iter()
    ...
    {
        let group: Vec<&engine::Match> = all_matches
            .iter()
            .filter(|m| m.settlement_tier == _tier)
            .collect();
        let all_correct = group
            .iter()
            .all(|m| m.fee_basis_points == m.settlement_tier.fee_basis_points());
        println!(
            "    {:?}: {} trades, fee compliance: {}",
            _tier,
            count,
    ...
    let tier_counts: std::collections::HashMap<SettlementPreference, usize> = all_matches
        .iter()
        .fold(std::collections::HashMap::new(), |mut acc, m| {
            *acc.entry(m.settlement_tier).or_insert(0) += 1;
            acc
    ...
    let settle_msg = format!(
        "Settle batch: {} trades, {} USD",
        all_matches.len(),
        total_value
    );
    let sig = tss
        .sign_message(&shares[0..3], settle_msg.as_bytes())
        .expect("TSS sign");
